...
class Local_predict_verify():
	...
	def load_clf_file(self,level,name):
		reader=pd.read_csv('/Users/pengchengliu/Documents/Master Thesis/data/qianhai/predict_local/'+level+'/'+name+'.csv',iterator=False,delimiter=',')#read prediction results
		d={}
	...
	def level_data(self):
		...
		for name in clf_name:
			...
			AUC_BBM_Level_One=0.753618206126
			AUC_BBM_Level_validation=0.757375004867
			AUC_BBM_Level_two=0.760720180713
...
########################## auc: 0.754049839922 > auc: 0.753618206126 the benchmark auc, correct 25 good users
				...
				#the second interval
				if i>=14000/5 and i <=16000/5:
					if diff>12000/5:
						column_dict[uid]=0#reverse
						r_lr+=1
				...
				#the third interval
				if i>=20000/5 and i <=23000/5:#25000/4 (more radical) will triger
					if diff>15000/5:
						column_dict[uid]=0#reverse
						r_lr+=1
...
#################################
				#the first interval
                  ...
                  #choose 
				if diff>3000/5+i*0.3:#or 
					if rank[uid][0]<448:#50,100,200
						column_dict[uid]=0
			...
			print( "auc increase:",(auc_score-AUC_BBM_Level_validation))#drop to auc: 0.753348228282 when one bad is estimated as good 
	...
	def print_diff(self,zero_diff,zero_index,one_diff,one_index):
		...
		for i in range(2000,6401):
			x.append(i)
			y.append(3000/5+i*0.3)
